...
void LoadPort_Simulator( int _nLP )
{
	...
	WRITE_DIGITAL(D_PORT1_CanMap + _nLP, 1, &nCommStatus);
	...
	while ( TRUE )
	{
		//LP_HOME , LP_LOAD , LP_UNLOAD , LP_CLAMP , LP_UNCLAMP , LP_CLOSE , LP_OPEN , LP_UNLOAD_AND_MAP
		...
		if ( nPORTx_Control >= 0 )
		{
			...
			switch ( nPORTx_Control )
			{
				...
				case LP_LOAD	:
					...
					if (READ_DIGITAL( D_LP_Carrier_Capacity, &nCommStatus ) == 0)	// ENUM: 25=0, 13=1
						strcpy( cMappingData , "1111111111111111111111111" );
					else
						strcpy( cMappingData , "1111111111111000000000000" );
					WRITE_STRING( S_PORT1_MAP_DATA + _nLP , cMappingData , &nCommStatus );
					break;
				case LP_UNLOAD	:
				case LP_UNLOAD_AND_MAP	:
					WRITE_DIGITAL( D_PORT1_DoorStatus + _nLP , DOOR_CLOSE_STS , &nCommStatus );
					if (READ_DIGITAL( D_LP_Carrier_Capacity, &nCommStatus ) == 0)	// ENUM: 25=0, 13=1
						strcpy( cMappingData , "1111111111111111111111111" );
					else

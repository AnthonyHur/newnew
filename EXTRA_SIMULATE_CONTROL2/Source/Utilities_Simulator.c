...
//	WRITE_DIGITAL( D_Bulkfill_SupplyReady , 1 , &nCommStatus );
	...
	if ( READ_DIGITAL( D_IonizerUsage , &nCommStatus ) == 1 )
...
////	WRITE_DIGITAL(D_WS_Fac_DI_Sts, 1, &nCommStatus);
////	WRITE_DIGITAL(D_WS_Fac_ACID_Sts, 1, &nCommStatus);
////	WRITE_DIGITAL(D_WS_Fac_HIDI_Sts, 1, &nCommStatus);
////	WRITE_DIGITAL(D_WS_ACID_Avail, 1, &nCommStatus);
////	WRITE_DIGITAL(D_WS_ACID_Drum1_Sts, 1, &nCommStatus);
////	WRITE_DIGITAL(D_WS_ACID_Drum2_Sts, 1, &nCommStatus);


	WRITE_ANALOG(A_FM_FFU_RPM, 1500, &nCommStatus);
	WRITE_ANALOG(A_FM_FFU_PRESS, 15, &nCommStatus);	
	WRITE_DIGITAL(D_EFFM_FFU_STATUS, 1, &nCommStatus);
